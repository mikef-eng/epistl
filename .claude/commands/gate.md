...
1. **Resolve the target.**
   ...
   If `gh pr view <n> --json mergeable -q .mergeable` is `CONFLICTING` (`main` moved and touched the same files), merge `main` into the head branch once with `gh pr update-branch <n>`. This is the only refresh; there is no automatic one. If it fails on conflicts, stop and report the conflicting files: they need a lane (or the human) to resolve them on the feature branch, then re-run `/gate`.
