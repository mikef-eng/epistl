...
- For lane PRs (`api` / `mobile` / `native`): read the PR `## Coverage` table. For each row, confirm the named test exists and exercises the claimed AC (read the test). Missing/wrong coverage → PR comment + label issue `blocked`; do not merge.
...
- Prefer squash merge; delete the branch after merge (`gh pr merge --squash --delete-branch`).
