...
- **Notification decrypt / shared session state**: content-free pushes are turned into preview text on-device by `apps/mobile/src/crypto/notificationDecrypt.ts` (pure TypeScript; storage, per-contact cross-process lock and single-envelope fetch are injected). Ratchet state must live in storage both the main app and the iOS NSE / Android background handler can reach (App Group container / Android shared storage), guarded by a per-contact lock and a generation counter (compare-and-set writes); after an extension decrypt the plaintext is stored by message id so the main app does not re-advance the ratchet. See [`docs/decisions/0022-notification-decrypt-shared-session-state.md`](../decisions/0022-notification-decrypt-shared-session-state.md). Concrete storage/lock (#250) and fetch endpoint (#249) are separate issues.
...
- **Queued-envelope peek** (issue #249, `apps/api/src/messages.rs`): `GET /api/messages/queued/{from_user_id}` (bearer-authenticated) returns `{from, body_b64, sent_at}` for the oldest message in the *caller's own* offline queue whose payload `from` matches, or 404 `not_found`. It is strictly read-only: it walks `epistl.offline.<caller>` with JetStream raw message get (`get_first_raw_message_by_subject`, bounded to 256 messages) and never creates a consumer (the stream is WorkQueue, so any consumer ack would remove the message), so the envelope is still delivered over `/ws`. Used by #173's decrypt-for-notification flow; `body_b64` is opaque to the server.
