...
Flow: Planner opens issues (`planning` → `ready`) → `/ship` dispatches the matching lane(s) → each lane opens a PR with a `## Coverage` table. One issue's PR targets `main`; a multi-issue feature stages its lane PRs onto `feat/<slug>` (light merge, no CI runs off `main`-targeted PRs) → `/gate` opens the single PR into `main` for a staged feature, `merge-gate` reviews the whole diff (+ `crypto-reviewer` on crypto/auth paths) and arms auto-merge, and GitHub squash-merges when the required `ci` check is green. Nobody waits on CI ([`docs/decisions/0023-staged-features-and-auto-merge.md`](docs/decisions/0023-staged-features-and-auto-merge.md)). Mobile, native, ui, and api lanes may run in parallel (per-worktree Postgres + NATS isolation; see issue #219).
...
6. Never poll CI with `sleep` / repeated `gh pr checks`, and never block an agent on CI. Lane agents push and stop; `merge-gate` arms `gh pr merge --squash --auto` and returns, and GitHub merges once the required `ci` check passes.
...
- [`0023-staged-features-and-auto-merge.md`](docs/decisions/0023-staged-features-and-auto-merge.md) — staged feature branches, one CI run per feature, auto-merge instead of waiting (supersedes 0021)
- [`0021-ci-cost-trip-wire.md`](docs/decisions/0021-ci-cost-trip-wire.md) — superseded by 0023 (CI stays; trip-wire not tripped)
...
| Path | Purpose |
| --- | --- |
| `.claude/agents/` | `planner`, `api-dev`, `mobile-dev`, `native-dev`, `ui`, `merge-gate`, `crypto-reviewer` |
