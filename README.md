...
| Object storage | MinIO (S3-compatible), via the `minio` docker-compose service — chosen for its S3-API compatibility so a future migration to a cloud object store (S3, R2, etc.) needs only a config change, not a client rewrite. Holds avatar image bytes; Postgres's `users.image TEXT` column only ever stores a reference (the API's own `/api/avatar/{user_id}` serving path), never the bytes or a raw MinIO/S3 URL. The API is the sole issuer of short-lived presigned upload/download URLs (never proxying image bytes itself) — see [`docs/decisions/0017-minio-object-storage-for-avatars.md`](docs/decisions/0017-minio-object-storage-for-avatars.md). The upload/serving endpoints themselves, the S3 client dependency, and bucket creation are a later issue; this repo state only stands up the service. |
...
Postgres, NATS, and MinIO are stood up and required for local development (see "Running the stack locally" below). ScyllaDB is documented here for orientation only — it is not yet stood up.
...
2. **Start Postgres, NATS, and MinIO** (from repo root):

   ```bash
   docker compose up -d
   ```

   This runs `postgres:16` on `localhost:5432` (configured from your `.env`; defaults: user/password/db all `epistl`), `nats:2-alpine` on `localhost:4222` (JetStream-enabled, monitoring on `8222`), and `quay.io/minio/minio` on `localhost:9000` (S3 API) / `localhost:9001` (web console, default credentials from your `.env`), each with a named volume so data survives restarts. (The `nats` service uses the `-alpine` tag rather than the bare `nats:2` tag purely so its healthcheck has a shell + `wget` to run against `8222`'s `/healthz` — it's still the same official `nats-io` image otherwise. MinIO's S3 API and web console ports are both published to the host, not just exposed internally, because presigned URLs handed to the mobile client must be reachable from outside the docker-compose network — see [`docs/decisions/0017-minio-object-storage-for-avatars.md`](docs/decisions/0017-minio-object-storage-for-avatars.md).) The upload/serving endpoints that actually use MinIO are a later issue; this step only stands the service up.
