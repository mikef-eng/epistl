...
| Transport | HTTP/WebSocket (the API's real, in-use transport). Quinn/QUIC has only been spiked, not adopted — see [`packages/quic-relay-client`](packages/quic-relay-client) above and issue #67; a real server-side QUIC listener and dual-stack QUIC/WS client racing remain undesigned future work, not committed to. Once a real QUIC listener exists, its TLS trust story (self-signed dev cert, no production cert-issuance pipeline) is a deliberately interim decision — see [`docs/decisions/0011-quic-dev-cert-trust-remains-dev-only.md`](docs/decisions/0011-quic-dev-cert-trust-remains-dev-only.md). |
